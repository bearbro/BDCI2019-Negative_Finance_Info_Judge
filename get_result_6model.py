...
total_prob_result = total_prob.argmax(axis=1)  # {0,1}
# todo 检查一下是否匹配
test_data = pd.read_csv(
    '/Users/brobear/Downloads/BDCI2019-Negative_Finance_Info_Judge/data/preprocess/bear_cv_data/cv_0/test.tsv')
# temp = pd.read_csv('/userhome/project/duibi/all_cv_0_result.csv')
# temp['entity']=temp['entity'].fillna('')
test_result_entity = pd.DataFrame(
    {'id': data_prob[0]['id'], 'negative': total_prob_result, 'key_entity': test_data['question']})
test_result_entity['negative'].sum()

# 整合答案 一个id对应1个实体 -》 一个id对应多个实体 即 n行合成1行
result_final = pd.DataFrame(columns=['id', 'negative', 'key_entity'])
neg_no_entity = pd.DataFrame(columns=['id', 'negative', 'key_entity'])
entity_filter_list = []  # 必在key_entity的实体
num = 0
total = 0
