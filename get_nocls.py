...
test_data['text'] = test_data.apply(lambda x: x['title'] + ' ' + x['text'] if x['title'] != x['text'] else x['text'],
                                    axis=1)
...
train_data['text'] = train_data.apply(lambda x: x['title'] + ' ' + x['text'] if x['title'] != x['text'] else x['text'],
                                      axis=1)
...
for index, item in train_data.iterrows():
    if item['negative'] == 0:
        ...
        for i in entity_list:
            train_entity = train_entity.append(
                pd.Series({'id': item['id'], 'text': item['text'], 'entity': i, 'negative': 0}), ignore_index=True)
    else:
        entity_list = item['entity'].split(';')
        ...
        for i in select_entity_list:
            if i not in key_entity_list:
                ...
                train_entity = train_entity.append(
                    pd.Series({'id': item['id'], 'text': item['text'], 'entity': i, 'negative': 1}), ignore_index=True)
...
for index, item in test_data.iterrows():
    ...
    if len(select_entity_list) == 0:
        print(entity_list)
    for i in select_entity_list:
        test_entity = test_entity.append(pd.Series({'id': item['id'], 'text': item['text'], 'entity': i}),
                                         ignore_index=True)
...
base_list = ['bert_ext_l2', 'bert_ext_l2_pretrain', 'bert_large_4v_lr2', 'bert_large_4v_lr3',
             'bert_large_4v_lr3_pretrain', 'bert_large_4v_lr3']
import numpy as np
...
def select(entity_list):  # 去除被蕴含的实体
    new_list = [i for i in entity_list]
    #     不扔掉不出现在text中的实体
    #     for i in entity_list:
